...
import java.util.concurrent.{ ConcurrentLinkedQueue, TimeUnit, Callable, Executor }
...
object Future {
  ...
  */
   ...
   */
  private trait BatchingExecutor extends Executor {
  ...
  private[concurrent] object InternalCallbackExecutor extends ExecutionContext {
    // delegate to a `BatchingExecutor`
    private val batchingExecutor = new BatchingExecutor with ExecutionContext {
    ...
    override def execute(runnable: Runnable) = batchingExecutor.execute(runnable)

    override def reportFailure(t: Throwable) = batchingExecutor.reportFailure(t)
  }
}
