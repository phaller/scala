...
object Props {
  ...
  final val defaultCreator: () ⇒ RichActor = () ⇒ throw new UnsupportedOperationException("No actor creator specified!")
...
//    case _: ActorKilledException         ⇒ Stop
   ...
   */
  def apply[T <: RichActor: ClassManifest]: Props =
    default.withCreator(implicitly[ClassManifest[T]].erasure.asInstanceOf[Class[_ <: RichActor]].newInstance)

  /**
   * Returns a Props that has default values except for "creator" which will be a function that creates an instance
   * of the supplied class using the default constructor
   */
  def apply(actorClass: Class[_ <: RichActor]): Props =
  ...
  def apply(faultHandler: FaultHandlingStrategy): Props =
    apply(new RichActor { def handle = { case _ ⇒ }}).withFaultHandler(faultHandler)
    
  /**
   * Returns a Props that has default values except for "creator" which will be a function that creates an instance
   * using the supplied thunk
   */
  def apply(creator: ⇒ RichActor): Props = default.withCreator(creator)
}

/**
 * ActorRef configuration object, this is thread safe and fully sharable
 */
case class Props(creator: () ⇒ RichActor = Props.defaultCreator,                 
   ...
   *  Scala API
   */
  def withCreator(c: ⇒ RichActor) = copy(creator = () ⇒ c)
